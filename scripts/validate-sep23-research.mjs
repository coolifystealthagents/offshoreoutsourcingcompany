...
const origin = process.env.VALIDATION_ORIGIN || 'http://127.0.0.1:39123';
const canonicalOrigin = 'https://offshoreoutsourcingcompany.com';
const slugs = [
  'philippines-customer-identity-handoff-continuity-study',
  'offshore-crm-suppression-propagation-integrity-research',
  'philippines-executive-calendar-metadata-minimization-study',
  'offshore-ecommerce-inventory-exception-lineage-research',
  'philippines-project-dependency-acceptance-traceability-study',
];
...
for (const slug of slugs) {
  ...
  assert.ok(html.includes('September 23, 2026'), `${path}: visible publication date`);
  assert.ok(html.includes('"datePublished":"2026-09-23"'), `${path}: structured datePublished`);
  assert.ok(!html.includes('"dateModified"'), `${path}: no unsupported dateModified`);
...
for (const path of new Set(datedLinks)) {
  ...
  if (html.includes('"datePublished":"2026-09-23"')) datedCount += 1;
}
assert.equal(datedCount, 5, 'exactly five Research articles dated 2026-09-23');
